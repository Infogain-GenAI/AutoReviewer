...
async function init() {
  ...
  if (!jiraKey.includes('-')) {
    if (!context.payload.pull_request) return setFailed('Partial Jira key can only be used in pull requests!')
    ...
    const inputs = [pullRequest.data.head.ref, pullRequest.data.title, pullRequest.data.body]
